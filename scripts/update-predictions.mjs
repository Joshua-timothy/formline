...
const forecastEnd = new Date(Date.now() + 14 * 86_400_000)
  .toISOString()
  .slice(0, 10);
const startYear =
...
async function getLeague(code, seasonKey) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const response = await fetch(urlFor(seasonKey, code), {
        headers: { "User-Agent": "PitchProbability/0.1" },
      });
      if (response.ok) {
        const payload = await response.json();
        if (!Array.isArray(payload.matches))
          throw new Error(
            `${code} ${seasonKey}: source returned no match list`,
          );
        return payload;
      }
...
function teamKey(name) {
  return String(name ?? "")
    ...
    .toLowerCase()
    .replace(
      /\b(fc|cf|afc|ac|as|sc|rc|rcd|cd|ud|sd|ss|us|calcio|football club)\b/g,
      " ",
    )
    .replace(/[^a-z0-9]+/g, " ")
    .trim()
...
function oddsValues(row) {
  ...
  for (const columns of priceSets) {
    ...
    if (
      prices.every(
        (price) => Number.isFinite(price) && price > 1.01 && price < 100,
      )
    ) {
...
function sourceXg(row) {
  ...
  return values.every(
    (value) => Number.isFinite(value) && value >= 0 && value <= 8,
  )
    ? values
    : null;
}
...
async function getOdds(division, seasonKey) {
  const response = await fetch(oddsUrlFor(seasonKey, division), {
    headers: { "User-Agent": "PitchProbability/0.1" },
  });
  if (!response.ok)
    throw new Error(`${division} ${seasonKey}: odds HTTP ${response.status}`);
  return parseCsv(await response.text()).flatMap((row) => {
    const date = oddsDate(row.Date),
      probabilities = oddsValues(row);
    if (!date || !row.HomeTeam || !row.AwayTeam || !probabilities) return [];
    return [
      {
        date,
        home: teamKey(row.HomeTeam),
        away: teamKey(row.AwayTeam),
        probabilities,
        xg: sourceXg(row),
      },
    ];
...
function oddsIndex(rows) {
  ...
  for (const row of rows.flat())
    index.set(`${row.date}|${row.home}|${row.away}`, row);
  return index;
}

function sourceRecord(match, index) {
  if (!index?.size) return null;
  const home = teamKey(match.team1),
    away = teamKey(match.team2);
  const fixture = new Date(`${match.date}T00:00:00Z`);
  for (const shift of [0, -1, 1]) {
    const date = new Date(fixture.getTime() + shift * 86_400_000)
      .toISOString()
      .slice(0, 10);
...
function performanceScore(match) {
  ...
  return score.map(
    (value, index) => value * 0.52 + match.sourceXg[index] * 0.48,
  );
}
...
function historicalWeight(date, asOf) {
  const daysAgo = Math.max(
    0,
    (Date.parse(`${asOf}T00:00:00Z`) - Date.parse(`${date}T00:00:00Z`)) /
      86_400_000,
  );
...
function buildStats(matches, asOf, fitIterations = 90) {
  const completed = matches
    .filter((match) => finalScore(match) && match.date < asOf)
    .sort((a, b) => a.date.localeCompare(b.date));
  const observations = completed.map((match) => ({
    match,
    score: performanceScore(match),
    weight: historicalWeight(match.date, asOf),
  }));
  const totalWeight =
    observations.reduce((total, item) => total + item.weight, 0) || 1;
  const homeGoals =
    observations.reduce(
  ...
  const awayGoals =
    observations.reduce(
      (total, item) => total + item.score[1] * item.weight,
      0,
    ) / totalWeight || 1.15;
  const names = new Set(
    completed.flatMap((match) => [teamId(match.team1), teamId(match.team2)]),
  );
  ...
  const profileFor = (team) =>
    teamProfile.get(team) ?? {
      currentGames: 0,
  ...
  for (const { match, weight } of observations) {
    ...
    for (const [team, venue] of [
      [teamId(match.team1), "homeWeight"],
      [teamId(match.team2), "awayWeight"],
    ]) {
      const profile = profileFor(team);
      if (current) profile.currentGames += 1;
      else if (seasonKey(match.date) === seasonKey(asOf) - 1)
        profile.previousSeasonGames += 1;
  ...
  const teamReliability = (team) => {
    ...
    const prior =
      profile.previousSeasonGames / (profile.previousSeasonGames + 18);
  ...
  const promotedSignalReliability = (team) =>
    isPromoted(team) ? clamp(teamReliability(team) * 1.2, 0.16, 0.72) : 1;
  const edgeReliability = (team) => {
    const profile = profileFor(team);
    const balancedSample = Math.min(profile.homeWeight, profile.awayWeight);
    return (
      clamp(balancedSample / (balancedSample + 9), 0, 0.58) *
      (profile.previousSeasonGames < 5 ? 0.65 : 1)
    );
  };
  const change = (map, team, amount) =>
    map.set(team, clamp((map.get(team) ?? 0) + amount, -0.9, 0.9));
  ...
  for (let iteration = 0; iteration < fitIterations; iteration++) {
    const attackGradient = new Map(),
      defenseGradient = new Map(),
      homeEdgeGradient = new Map();
    const add = (map, team, value) =>
      map.set(team, (map.get(team) ?? 0) + value);
    for (const { match, score, weight } of observations) {
      const homeTeam = teamId(match.team1),
        awayTeam = teamId(match.team2);
      const edge = homeEdge.get(homeTeam) ?? 0;
      const expectedHome = clamp(
        homeGoals *
          Math.exp(
            (attack.get(homeTeam) ?? 0) -
              (defense.get(awayTeam) ?? 0) +
              edge * 0.55,
          ),
        0.2,
        4.5,
      ...
      const expectedAway = clamp(
        awayGoals *
          Math.exp(
            (attack.get(awayTeam) ?? 0) -
              (defense.get(homeTeam) ?? 0) -
              edge * 0.25,
          ),
        0.2,
        4.5,
      );
      const homeResidual = weight * (score[0] - expectedHome),
        awayResidual = weight * (score[1] - expectedAway);
      add(attackGradient, homeTeam, homeResidual);
      add(defenseGradient, awayTeam, -homeResidual);
      add(attackGradient, awayTeam, awayResidual);
      add(defenseGradient, homeTeam, -awayResidual);
      add(
        homeEdgeGradient,
        homeTeam,
        homeResidual * 0.55 - awayResidual * 0.25,
      );
    }
    for (const team of names) {
      change(
        ...
        team,
        0.38 *
          ((attackGradient.get(team) ?? 0) / totalWeight -
            0.36 * (attack.get(team) ?? 0)),
      );
      change(
        defense,
        team,
        0.38 *
          ((defenseGradient.get(team) ?? 0) / totalWeight -
            0.36 * (defense.get(team) ?? 0)),
      );
      homeEdge.set(
        team,
        clamp(
          (homeEdge.get(team) ?? 0) +
            0.18 *
              ((homeEdgeGradient.get(team) ?? 0) / totalWeight -
                1.45 * (homeEdge.get(team) ?? 0)),
          -0.22,
          0.22,
        ),
      );
  ...
  for (const team of names)
    homeEdge.set(
      team,
      clamp((homeEdge.get(team) ?? 0) * edgeReliability(team), -0.12, 0.12),
    );
  ...
  const rating = (team) =>
    ratings.get(team) ?? (isPromoted(team) ? 1465 : 1500);
  let activeSeason;
  for (const match of completed) {
    const matchSeason = seasonKey(match.date);
    if (activeSeason !== undefined && matchSeason !== activeSeason) {
      for (const [team, value] of ratings)
        ratings.set(team, 1500 + (value - 1500) * 0.82);
    }
    activeSeason = matchSeason;
    const homeTeam = teamId(match.team1),
      awayTeam = teamId(match.team2);
    const [home, away] = finalScore(match),
      homeRating = rating(homeTeam),
      awayRating = rating(awayTeam);
    const expectedHome =
      1 / (1 + Math.pow(10, -(homeRating + 58 - awayRating) / 400));
    ...
    lastPlayed.set(homeTeam, match.date);
    lastPlayed.set(awayTeam, match.date);
  }

  const form = new Map();
  const addForm = (team, attackResidual, defenseResidual, weight) => {
    const value = form.get(team) ?? {
      attack: 0,
      defense: 0,
      games: 0,
      weight: 0,
    ...
    if (value.games < 6) {
      value.attack += attackResidual * weight;
      value.defense += defenseResidual * weight;
      value.games += 1;
      value.weight += weight;
      form.set(team, value);
    }
  ...
  for (const match of [...completed].reverse()) {
    ...
    const expectedHome = clamp(
      homeGoals *
        Math.exp((attack.get(homeTeam) ?? 0) - (defense.get(awayTeam) ?? 0)),
      0.2,
      4.5,
    );
    const expectedAway = clamp(
      awayGoals *
        Math.exp((attack.get(awayTeam) ?? 0) - (defense.get(homeTeam) ?? 0)),
      0.2,
      4.5,
    );
    const daysAgo = Math.max(
      ...
      (Date.parse(`${asOf}T00:00:00Z`) -
        Date.parse(`${match.date}T00:00:00Z`)) /
        86_400_000,
    );
    const recency = Math.exp(-daysAgo / 55);
    addForm(
      homeTeam,
      Math.log((home + 0.65) / (expectedHome + 0.65)),
      Math.log((away + 0.65) / (expectedAway + 0.65)),
      recency,
    );
    addForm(
      awayTeam,
  ...
  for (const value of form.values()) {
    ...
    value.attack = clamp(
      (value.attack / value.weight) * shrinkage,
      -0.35,
      0.35,
    );
    value.defense = clamp(
      (value.defense / value.weight) * shrinkage,
      -0.35,
      0.35,
    );
  }
  ...
  const addCurrentPerformance = (team, attack, defense, weight) => {
    ...
    if (value.games < 6) {
      value.attack += attack * weight;
      value.defense += defense * weight;
      value.weight += weight;
      value.games += 1;
      currentPerformance.set(team, value);
    }
  };
  for (const match of [...completed].reverse()) {
    ...
    addCurrentPerformance(
      homeTeam,
      Math.log((home + 0.5) / (homeGoals + 0.5)),
      Math.log((away + 0.5) / (awayGoals + 0.5)),
      recency,
    );
    addCurrentPerformance(
      awayTeam,
  ...
  const addCurrentXg = (team, attack, defense, weight) => {
    ...
    if (value.games < 5) {
      ...
      value.weight += weight;
      value.games += 1;
      currentXg.set(team, value);
    }
  };
  for (const match of [...completed].reverse()) {
    if (
      seasonKey(match.date) !== seasonKey(asOf) ||
      !Array.isArray(match.sourceXg)
    )
      continue;
    const homeTeam = teamId(match.team1),
      awayTeam = teamId(match.team2);
    ...
    addCurrentXg(
      homeTeam,
      Math.log((match.sourceXg[0] + 0.35) / (homeGoals + 0.35)),
      Math.log((match.sourceXg[1] + 0.35) / (awayGoals + 0.35)),
      recency,
    );
    addCurrentXg(
      awayTeam,
  ...
  const currentTeamCount =
    [...teamProfile.values()].filter((profile) => profile.currentGames > 0)
      .length || 1;
  const currentMatchday =
    [...teamProfile.values()].reduce(
      (total, profile) => total + profile.currentGames,
      0,
    ) / currentTeamCount;
  return {
    homeGoals,
...
function strengthGoals(home, away, stats) {
  const homeTeam = teamId(home),
    awayTeam = teamId(away);
  const homeReliability = stats.isPromoted(homeTeam)
    ? stats.teamReliability(homeTeam)
    : 1;
  const awayReliability = stats.isPromoted(awayTeam)
    ? stats.teamReliability(awayTeam)
    : 1;
  const homeTransition = stats.divisionTransition(homeTeam),
    awayTransition = stats.divisionTransition(awayTeam);
...
function formGoals(home, away, stats) {
  ...
  const hp = stats.currentPerformance.get(homeTeam) ?? {
    attack: 0,
    defense: 0,
  };
  const ap = stats.currentPerformance.get(awayTeam) ?? {
    attack: 0,
    defense: 0,
  };
  const hx = stats.currentXg.get(homeTeam) ?? { attack: 0, defense: 0 };
  const ax = stats.currentXg.get(awayTeam) ?? { attack: 0, defense: 0 };
  const homeReliability = stats.teamReliability(homeTeam),
    awayReliability = stats.teamReliability(awayTeam);
  const homeFormReliability =
      stats.currentFormReliability(homeTeam) *
      stats.promotedSignalReliability(homeTeam),
    awayFormReliability =
      stats.currentFormReliability(awayTeam) *
      stats.promotedSignalReliability(awayTeam);
  const homeXgReliability =
      stats.xgReliability(homeTeam) * stats.promotedSignalReliability(homeTeam),
    awayXgReliability =
      stats.xgReliability(awayTeam) * stats.promotedSignalReliability(awayTeam);
  return {
    home: clamp(
      baseline.home *
        Math.exp(
          h.attack * homeReliability -
            a.defense * awayReliability +
            1.08 *
              (hp.attack * homeFormReliability -
                ap.defense * awayFormReliability) +
            0.72 *
              (hx.attack * homeXgReliability - ax.defense * awayXgReliability),
        ),
      0.25,
      4.0,
    ),
    away: clamp(
      baseline.away *
        Math.exp(
          a.attack * awayReliability -
            h.defense * homeReliability +
            1.08 *
              (ap.attack * awayFormReliability -
                hp.defense * homeFormReliability) +
            0.72 *
              (ax.attack * awayXgReliability - hx.defense * homeXgReliability),
        ),
      0.25,
      4.0,
    ),
...
function eloGoals(home, away, stats) {
  const homeTeam = teamId(home),
    awayTeam = teamId(away);
  const homeRating = stats.ratings.get(homeTeam) ?? 1500,
    awayRating = stats.ratings.get(awayTeam) ?? 1500;
  const ratingDifference = homeRating - awayRating;
  const teamHomeEdge = (stats.homeEdge.get(homeTeam) ?? 0) * 85;
  const homeWinStrength =
    1 / (1 + Math.pow(10, -(ratingDifference + 58 + teamHomeEdge) / 400));
  const totalGoals = stats.homeGoals + stats.awayGoals;
  const homeShare = clamp(
    stats.homeGoals / totalGoals + (homeWinStrength - 0.5) * 0.55,
    0.2,
    0.8,
  );
...
function componentGoals(home, away, stats) {
  return {
    strength: strengthGoals(home, away, stats),
    form: formGoals(home, away, stats),
    current: currentPerformanceGoals(home, away, stats),
    elo: eloGoals(home, away, stats),
  };
}

function blendGoals(components, weights) {
  return ["home", "away"].reduce((result, side) => {
    result[side] = ["strength", "form", "current", "elo"].reduce(
      (sum, name) => sum + components[name][side] * weights[name],
      0,
    );
    return result;
  }, {});
...
function adaptiveWeights(baseWeights, stats, match) {
  ...
  const matchupReliability =
    (stats.teamReliability(teamId(match.team1)) +
      stats.teamReliability(teamId(match.team2))) /
    2;
  const currentFormEvidence =
    (stats.currentFormReliability(teamId(match.team1)) +
      stats.currentFormReliability(teamId(match.team2))) /
    2;
  const currentXgEvidence =
    (stats.xgReliability(teamId(match.team1)) +
      stats.xgReliability(teamId(match.team2))) /
    2;
  const multipliers = {
    ...
    form:
      0.45 +
      stage * 0.75 +
      currentFormEvidence * 1.25 +
      currentXgEvidence * 0.42,
    current: 0.22 + currentFormEvidence * 1.65 + currentXgEvidence * 0.85,
    elo: 1.15 - stage * 0.12,
  };
  const raw = Object.fromEntries(
    Object.entries(baseWeights).map(([name, weight]) => [
      name,
      weight * multipliers[name],
    ]),
  );
...
function restProfile(team, fixtureDate, stats) {
  ...
  const previous = (stats.teamFixtures.get(teamId(team)) ?? []).filter(
    (date) => Date.parse(`${date}T00:00:00Z`) < fixtureTime,
  );
  const daysSinceLast = previous.length
    ? Math.round(
        (fixtureTime - Date.parse(`${previous.at(-1)}T00:00:00Z`)) / 86_400_000,
      )
    : 7;
  const matchesWithin = (days) =>
    previous.filter(
      (date) =>
        fixtureTime - Date.parse(`${date}T00:00:00Z`) <= days * 86_400_000,
    ).length;
    ...
    daysSinceLast <= 2
      ...
      : daysSinceLast === 3
        ...
        : daysSinceLast === 4
          ...
          : 0;
  const congestion =
    Math.max(0, matches10 - 2) * 0.008 + Math.max(0, matches21 - 4) * 0.005;
  return {
...
function applyRestAdjustment(xg, match, stats) {
  ...
  const homeAdjustment = clamp(
    -home.fatigue * 0.76 + away.fatigue * 0.42,
    -0.055,
    0.04,
  );
  const awayAdjustment = clamp(
    -away.fatigue * 0.76 + home.fatigue * 0.42,
    -0.055,
    0.04,
  );
  return {
    xg: {
      home: clamp(xg.home * (1 + homeAdjustment), 0.25, 3.5),
      away: clamp(xg.away * (1 + awayAdjustment), 0.25, 3.5),
    },
    rest: {
      home: home.days,
      away: away.days,
      homeFatigue: Number(home.fatigue.toFixed(3)),
      awayFatigue: Number(away.fatigue.toFixed(3)),
      homeMatches21: home.matches21,
      awayMatches21: away.matches21,
    },
  };
}
...
function scoreMatrix(xg) {
  ...
  for (const { h, a, probability: rawProbability } of cells) {
    ...
    if (probability > best.probability)
      best = { home: h, away: a, probability };
  }
...
function blendProbabilities(model, market, marketWeight) {
  ...
  const blended = model.map(
    (value, index) => value * (1 - marketWeight) + market[index] * marketWeight,
  );
...
function probabilityScore(predicted, actual) {
  return (
    predicted.reduce(
      (sum, value, index) => sum + Math.pow(value - actual[index], 2),
      0,
    ) / 3
  );
}
...
function predict(match, league, stats, diagnostic, odds) {
  ...
  const adjusted = applyRestAdjustment(
    blendGoals(components, weights),
    match,
    stats,
  );
  const xg = adjusted.xg;
  const { home, draw, away, btts, over15, over25, over35, best } =
    scoreMatrix(xg);
  const marketOdds = marketProbabilities(match, odds);
  const signal = (team) => {
    const value = stats.currentXg.get(teamId(team)) ?? {
      attack: 0,
      defense: 0,
      games: 0,
    };
    return {
      attack: Number(value.attack.toFixed(3)),
      defense: Number(value.defense.toFixed(3)),
      games: value.games ?? 0,
      reliability: Number(stats.xgReliability(teamId(team)).toFixed(3)),
    };
  };
  const marketWeight = marketOdds
    ? clamp(
        (diagnostic?.marketCalibration?.weight ?? 0.35) *
          (1.08 - adaptive.stage * 0.12),
        0.16,
        0.65,
      )
    : 0;
  const [resultHome, resultDraw, resultAway] = blendProbabilities(
    [home, draw, away],
    marketOdds,
    marketWeight,
  );
  ...
  const dnbHome = resultHome / (resultHome + resultAway),
    dnbAway = resultAway / (resultHome + resultAway);
  const result = [
    ["Home", resultHome],
    ["Draw", resultDraw],
    ["Away", resultAway],
  ...
  const dc = [
    ["1X", resultHome + resultDraw],
    ["12", resultHome + resultAway],
    ["X2", resultDraw + resultAway],
  ].sort((a, b) => b[1] - a[1])[0];
  const choose = (yes, no, yesLabel, noLabel) =>
    yes >= no ? [yesLabel, yes] : [noLabel, no];
  return {
    id: `${league.id}-${match.date}-${match.team1}-${match.team2}`
      .replace(/[^a-z0-9]+/gi, "-")
      .toLowerCase(),
    league: league.name,
    leagueId: league.id,
    ...
    confidence: marketOdds
      ? "Market-calibrated model"
      : diagnostic?.matches >= 120
        ? "Backtested model"
        : "Early-season model",
    model: {
      weights,
      stage: adaptive.stage,
      matchupReliability: adaptive.matchupReliability,
      currentFormEvidence: adaptive.currentFormEvidence,
      currentXgEvidence: adaptive.currentXgEvidence,
      currentXgSignal: { home: signal(match.team1), away: signal(match.team2) },
      componentXg: Object.fromEntries(
        Object.entries(components).map(([name, values]) => [
          name,
          {
            home: Number(values.home.toFixed(2)),
            away: Number(values.away.toFixed(2)),
          },
        ]),
      ),
      restDays: adjusted.rest,
      marketCalibration: {
        available: Boolean(marketOdds),
        weight: Number(marketWeight.toFixed(3)),
      },
      validation: diagnostic
        ? {
            matches: diagnostic.matches,
            brier: diagnostic.brier,
            logLoss: diagnostic.logLoss,
            ece: diagnostic.calibration.ece,
          }
        : null,
    },
    markets: [
      ...
      { name: "Double chance", ...market(dc[0], dc[1]) },
      {
        name: "Draw no bet",
        ...market(
          dnbHome >= dnbAway ? "Home" : "Away",
          Math.max(dnbHome, dnbAway),
        ),
      },
      {
        name: "Over / Under 1.5",
        ...market(...choose(over15, under15, "Over 1.5", "Under 1.5")),
      },
      {
        name: "Over / Under 2.5",
        ...market(...choose(over25, under25, "Over 2.5", "Under 2.5")),
      },
      {
        name: "Over / Under 3.5",
        ...market(...choose(over35, under35, "Over 3.5", "Under 3.5")),
      },
      {
        name: "Both teams to score",
        ...market(...choose(btts, 1 - btts, "Yes", "No")),
      },
      {
        name: "Correct score",
        ...market(`${best.home}-${best.away}`, best.probability),
      },
      {
        name: "Home team goals",
        ...market(
          ...choose(homeOver15, 1 - homeOver15, "Over 1.5", "Under 1.5"),
        ),
      },
      {
        name: "Away team goals",
        ...market(
          ...choose(awayOver05, 1 - awayOver05, "Over 0.5", "Under 0.5"),
        ),
      },
    ],
  };
...
function backtest(matches, odds) {
  const completed = matches
    .filter((match) => finalScore(match) && match.date < today)
    .sort((a, b) => a.date.localeCompare(b.date));
  ...
  const componentBrier = Object.fromEntries(
    Object.entries(brier).map(([name, score]) => [name, score / sample.length]),
  );
  const ensemblePrior = { strength: 1.8, form: 0.8, current: 0.72, elo: 0.55 };
  const inverse = ["strength", "form", "current", "elo"].map((name) => [
    name,
    ensemblePrior[name] / (componentBrier[name] + 0.01),
  ]);
  const totalInverse = inverse.reduce((sum, [, value]) => sum + value, 0);
  const rawWeights = Object.fromEntries(
    inverse.map(([name, value]) => [name, value / totalInverse]),
  );
  ...
  for (const observation of observations) {
    ...
    const xg = applyRestAdjustment(
      blendGoals(observation.components, weights),
      observation.fixture,
      observation.stats,
    ).xg;
    ...
    modelObservations.push({
      ...observation,
      predicted: [matrix.home, matrix.draw, matrix.away],
      market: marketProbabilities(observation.fixture, odds),
    });
  }
  const matchedMarket = modelObservations.filter(
    (observation) => observation.market,
  );
  ...
  if (matchedMarket.length >= 9) {
    ...
    const best = scored.reduce((winner, candidate) =>
      candidate.brier < winner.brier ? candidate : winner,
    );
    ...
    marketCalibration = {
      ...
      modelBrier: Number(
        (
          matchedMarket.reduce(
            (sum, observation) =>
              sum + probabilityScore(observation.predicted, observation.actual),
            0,
          ) / matchedMarket.length
        ).toFixed(3),
      ),
  ...
  for (const observation of modelObservations) {
    const predicted = blendProbabilities(
      observation.predicted,
      observation.market,
      marketCalibration.weight,
    );
    const outcome = observation.actual.indexOf(1);
    brierScore +=
      predicted.reduce(
        (sum, value, index) =>
          sum + Math.pow(value - observation.actual[index], 2),
        0,
      ) / 3;
    ...
    bin.correct += chosen === outcome ? 1 : 0;
  }
  const ece = bins.reduce(
    (total, bin) =>
      bin.count
        ? total +
          (bin.count / sample.length) *
            Math.abs(bin.confidence / bin.count - bin.correct / bin.count)
        : total,
    0,
  );
  const weights = Object.fromEntries(
    Object.entries(rawWeights).map(([name, value]) => [
      name,
      Math.round(value * 1000) / 1000,
    ]),
  );
  const components = Object.fromEntries(
    Object.entries(componentBrier).map(([name, value]) => [
      name,
      Math.round(value * 1000) / 1000,
    ]),
  );
  return {
    ...
    accuracy: Math.round((correct / sample.length) * 1000) / 10,
    brier: Math.round((brierScore / sample.length) * 1000) / 1000,
    logLoss: Math.round((logLoss / sample.length) * 1000) / 1000,
    calibration: { ece: Math.round(ece * 1000) / 1000 },
    components,
    weights,
    marketCalibration,
  };
...
for (const [id, name, code, oddsDivision] of leagues) {
  try {
    const [current, previous, older, currentOdds, previousOdds, olderOdds] =
      await Promise.all([
        getLeague(code, season),
        getLeague(code, previousSeason),
        getLeague(code, twoSeasonsAgo),
        getOdds(oddsDivision, season).catch(() => []),
        getOdds(oddsDivision, previousSeason).catch(() => []),
        getOdds(oddsDivision, twoSeasonsAgo).catch(() => []),
      ]);
    const odds = oddsIndex([olderOdds, previousOdds, currentOdds]);
    const history = enrichMatches(
      [
        ...(older.matches ?? []),
        ...(previous.matches ?? []),
        ...(current.matches ?? []),
    ...
    const upcoming = (current.matches ?? []).filter(
      (match) =>
        match.date >= today && match.date <= forecastEnd && !finalScore(match),
    );
    results.push({
      id,
      name,
      predictions: upcoming.map((match) =>
        predict(match, { id, name }, stats, diagnostic, odds),
      ),
      diagnostic,
      error: null,
    });
...
if (new Set(ids).size !== ids.length)
  throw new Error("Refusing to publish a feed with duplicate fixture IDs.");
if (
  payload.predictions.some(
    (prediction) =>
      !Number.isFinite(prediction.xg.home) ||
      !Number.isFinite(prediction.xg.away),
  )
)
  throw new Error(
    "Refusing to publish a feed with invalid expected-goal values.",
  );
if (
  payload.predictions.some((prediction) => {
    ...
    return (
      ...
      Math.abs(values.reduce((sum, value) => sum + value, 0) - 100) > 0.2
    );
  })
)
...
if (!payload.predictions.length && failedLeagues.length) {
  throw new Error(
    `Refusing to replace the last good feed: no predictions were produced and ${failedLeagues.length} source${failedLeagues.length === 1 ? "" : "s"} failed.`,
  );
}
...
console.log(
  `Wrote ${payload.predictions.length} predictions for ${season} to ${output}`,
);
